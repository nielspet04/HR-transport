...
async function refresh(newUpload=false){const month=newUpload?null:matching?.month;const response=await fetch('/api/state');if(!response.ok)throw Error('Laden mislukt.');state=await response.json();if(state.view!=='routes')throw Error('Herstart het dashboard om het nieuwe routebeheer te openen.');const selected=state.matching_runs?.find(r=>r.month===month);if(selected&&selected.id!==state.matching?.run_id){const result=await fetch(`/api/matching/run?id=${selected.id}`);if(!result.ok)throw Error('Maand laden mislukt.');state.matching=await result.json();}render();}
...
function render(){
  ...
  selector($('renameWorker'),state.workers);selector($('movingWorker'),state.workers);renderRows();renderMove();matching=state.matching;renderMatching();renderTariffs();renderLocationAddresses();renderAddresses();renderDistanceRoutes();renderAutomaticRoutes();renderTransferOverview();showView(activeView);
}
function renderRows(){
...
function renderMatching(){
  ...
  if(!matching){$('matchingSummary').textContent='Nog geen bestand verwerkt. Upload hierboven de rechtstreekse Pl@net-export.';$('agentShifts').replaceChildren();$('matchingWarning').hidden=true;$('calculationSummary').textContent='';$('calculationWarning').textContent='Nog geen berekening.';return;}
...
$('matchingMonth').addEventListener('change',async()=>{try{const response=await fetch(`/api/matching/run?id=${encodeURIComponent($('matchingMonth').value)}`);if(!response.ok)throw Error('Maand laden mislukt.');matching=await response.json();renderMatching();}catch(error){message(error.message,true);}});
...
function renderAutomaticRoutes(){
  ...
  const status=`Automatische afstanden: ${auto.enabled?'aan':'uit'} · ${auto.status||'IDLE'} · ${auto.report?.geocoded||0} adressen · ${auto.report?.routes||0} nieuwe routes${auto.report?.message?' · '+auto.report.message:''}`;
